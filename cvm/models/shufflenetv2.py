...
class ShuffleBlockV2(nn.Module):
    def __init__(self, inp, oup, stride: int = 1, dilation: int = 1, ratio: float = None):
        super().__init__()
...
class SDShuffleBlockV2(nn.Module):
    def __init__(self, inp, oup, stride: int = 1, dilation: int = 1, ratio: float = 0.75):
        super().__init__()
        ...
        if stride != 1:
            self.branch1 = nn.Sequential(OrderedDict([
                ('dwconv', blocks.GaussianBlurBN(self.inp, stride=self.stride, dilation=self.dilation)),
                ('1x1', blocks.Conv2d1x1Block(self.inp, self.oup))
            ]))

        self.branch2 = nn.Sequential(OrderedDict([
            ('1x1-1', blocks.Conv2d1x1Block(self.inp, self.oup)),
            ('dwconv', blocks.SDDCBN(self.oup, ratio=ratio, dilation=self.dilation) if stride == 1 else blocks.GaussianBlurBN(self.oup, stride=self.stride, dilation=self.dilation)),
            ('1x1-2', blocks.Conv2d1x1Block(self.oup, self.oup))
        ]))
...
class ShuffleNetV2(nn.Module):
    def __init__(
        ...
        **kwargs: Any
        ...
        self.features = nn.Sequential(OrderedDict([
            ...
            ('stage2', self.make_layers(repeats[0], channels[0], channels[1], dilations[1], [15/16, 14/16])),
            ('stage3', self.make_layers(repeats[1], channels[1], channels[2], dilations[2], [12/16, 8/16])),
            ('stage4', self.make_layers(repeats[2], channels[2], channels[3], dilations[3], [4/16, 1/16])),
        ]))
    ...
    def make_layers(self, repeat, inp, oup, dilation, ratios):
        ...
        for i in range(repeat - 1):
            layers.append(self.block(oup, oup, dilation=dilation, ratio=ratios[i]))
...
@export
@config(url='https://github.com/ffiirree/cv-models/releases/download/v0.0.1-shufflenets-weights/sd_shufflenet_v2_x2_0-d942d6d3.pth')
