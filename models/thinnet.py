...
class DuplicateConvBlock(nn.Module):
    ...
    def _make_layers(self):
        conv2d = nn.Conv2d(self.channels, self.channels, self.kernel_size,
...
class ThinNet(nn.Module):
    ...
    def _make_blocks(self):
        ...
        for idx, _ in enumerate(self.filters):
            ...
            blocks.append(ConvBlock(self.in_channels, self.filters[0], 3, bn=self.bn) if idx == 0 else ConvBlock(
                self.filters[idx - 1], self.filters[idx], 3, bn=self.bn))
            for _ in range(self.n_blocks[idx]):
                blocks.append(DuplicateConvBlock(
                    self.filters[idx], n_layers=self.n_layers[idx], bn=self.bn))
